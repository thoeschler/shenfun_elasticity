...
from shenfun import comm, FunctionSpace, TensorProductSpace, \
    VectorSpace, Function


def get_dimensionless_values(dom, boundary_conditions, body_forces,
                             material_parameters, nondim_disp, nondim_length,
                             nondim_mat_param, u_ana=None):
    '''
    ...
    dom_dimless = np.array(dom) / nondim_length

    # dimensionless material_parameters
    if len(material_parameters) == 2:  # cauchy elasticity
        lambd, mu = np.array(material_parameters) / nondim_mat_param
        ...
        c1, c2, c3, c4, c5 = np.array(material_parameters)[2:] \
            / nondim_mat_param / nondim_length ** 2
        # wrap material parameters in a tuple
        material_parameters_dimless = lambd, mu, c1, c2, c3, c4, c5
    else:
        raise NotImplementedError(
            'Number of material parameters must be two or seven'
            )
    ...
    for bcs_comp in boundary_conditions:  # bcs for each component
        ...
        for bc in bcs_comp:  # bc in one direction for one component
            ...
            if bc_type == 'DIRICHLET':
                # dimensionless bc
                bc_dimless = []
                for component in bc:  # coordinate transformation
...
def get_dimensionful_values(u_hat_dimless, boundary_conditions,
                            nondim_disp, nondim_length):
    '''
    ...
    dom_dimless = [u_hat_dimless.function_space().spaces[i].bases[i].domain
                   for i in range(dim)]

    # dimensionful domain
    dom = [
            [
                dom_dimless[i][j]*nondim_length for j in range(2)
                ]
            for i in range(dim)
        ]
    ...
    for i in range(dim):  # nb of displacement components
        ...
        for j in range(dim):  # nb of FunctionSpaces for each component
            basis = FunctionSpace(N[j], domain=dom[j], family='legendre',
    ...
    for i in range(dim):
        # u has the same expansions coefficients as u_hat
        u_hat[i] = nondim_disp*u_hat_dimless[i]
